package com.marsn.minitalk.ui.mocks.messageContacts

import com.marsn.minitalk.model.Contact
import com.marsn.minitalk.model.Message
import com.marsn.minitalk.model.MessageContact

val messagesContactsMock: List<MessageContact> = listOf(

    MessageContact(
        contact = Contact(
            name = "Maria Santos",
            phoneNumber = "+351 910000001",
            email = "maria.santos@example.com",
            address = "Rua das Flores 12, Lisboa",
            profilePicture = "https://picsum.photos/200?1"
        ),
        message = Message(
            conversationId = 1L,
            senderId = 101L,
            text = "Bom dia! Como você está?",
            timestamp = 1737008000000L
        )
    ),

    MessageContact(
        contact = Contact(
            name = "João Matos",
            phoneNumber = "+351 910000002",
            email = "joao.matos@example.com",
            address = "Av. Central 55, Porto",
            profilePicture = "https://picsum.photos/200?2"
        ),
        message = Message(
            conversationId = 2L,
            senderId = 202L,
            text = "Chegou em casa já?",
            timestamp = 1737008060000L
        )
    ),

    MessageContact(
        contact = Contact(
            name = "Ana Ferreira",
            phoneNumber = "+351 910000003",
            email = "ana.ferreira@example.com",
            address = "Rua do Sol 89, Coimbra",
            profilePicture = "https://picsum.photos/200?3"
        ),
        message = Message(
            conversationId = 3L,
            senderId = 303L,
            text = "Me avisa quando puder falar.",
            timestamp = 1737008120000L
        )
    ),

    MessageContact(
        contact = Contact(
            name = "Pedro Almeida",
            phoneNumber = "+351 910000004",
            email = "pedro.almeida@example.com",
            address = "Travessa das Oliveiras 7, Faro",
            profilePicture = "https://picsum.photos/200?4"
        ),
        message = Message(
            conversationId = 4L,
            senderId = 404L,
            text = "Vamos hoje à noite?",
            timestamp = 1737008180000L
        )
    ),

    MessageContact(
        contact = Contact(
            name = "Carla Rodrigues",
            phoneNumber = "+351 910000005",
            email = "carla.rodrigues@example.com",
            address = "Rua Verde 3, Braga",
            profilePicture = "https://picsum.photos/200?5"
        ),
        message = Message(
            conversationId = 5L,
            senderId = 505L,
            text = "Obrigada pela ajuda mais cedo! 😊",
            timestamp = 1737008240000L
        )
    ),

    MessageContact(
        contact = Contact(
            name = "Miguel Costa",
            phoneNumber = "+351 910000006",
            email = "miguel.costa@example.com",
            address = "Rua Azul 44, Aveiro",
            profilePicture = "https://picsum.photos/200?6"
        ),
        message = Message(
            conversationId = 6L,
            senderId = 606L,
            text = "Tá vendo isso aqui?",
            timestamp = 1737008300000L
        )
    ),

    MessageContact(
        contact = Contact(
            name = "Beatriz Silva",
            phoneNumber = "+351 910000007",
            email = "beatriz.silva@example.com",
            address = "Rua da Lapa 22, Lisboa",
            profilePicture = "https://picsum.photos/200?7"
        ),
        message = Message(
            conversationId = 7L,
            senderId = 707L,
            text = "Te mandei o documento.",
            timestamp = 1737008360000L
        )
    ),

    MessageContact(
        contact = Contact(
            name = "Rafael Martins",
            phoneNumber = "+351 910000008",
            email = "rafael.martins@example.com",
            address = "Rua dos Lírios 16, Guimarães",
            profilePicture = "https://picsum.photos/200?8"
        ),
        message = Message(
            conversationId = 8L,
            senderId = 808L,
            text = "Cheguei! 😄",
            timestamp = 1737008420000L
        )
    ),

    MessageContact(
        contact = Contact(
            name = "Sofia Carvalho",
            phoneNumber = "+351 910000009",
            email = "sofia.carvalho@example.com",
            address = "Rua da Aurora 9, Setúbal",
            profilePicture = "https://picsum.photos/200?9"
        ),
        message = Message(
            conversationId = 9L,
            senderId = 909L,
            text = "Pode falar agora?",
            timestamp = 1737008480000L
        )
    ),

    MessageContact(
        contact = Contact(
            name = "Tiago Ribeiro",
            phoneNumber = "+351 910000010",
            email = "tiago.ribeiro@example.com",
            address = "Rua Nova 31, Sintra",
            profilePicture = "https://picsum.photos/200?10"
        ),
        message = Message(
            conversationId = 10L,
            senderId = 1001L,
            text = "Tô chegando aí.",
            timestamp = 1737008540000L
        )
    )
)
